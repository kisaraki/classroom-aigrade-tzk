...
學籍生效期間依 D-11 採臺北業務日期，含起點、不含終點；終點空值表示尚未結束。同一學生的有效學籍區間不得重疊，同班座號在重疊期間不得重複。評量參與紀錄在名單確認時固定當次學籍、班級及排名資格快照，不得在讀取歷史資料時用目前學籍重建。
...
依 2026-09-23 核准的 D-03，評量開始後轉入者自下一次定期評量參加本校評量；不得回補為已開始評量的本校參與者。開始前新增的參與紀錄仍須確認名單並保存當時快照。
...
公開查詢期限與資料保存期限必須分別記錄，且公開查詢期限不得晚於資料保存期限。依 2026-09-25 核准 D-05，延長公開查詢必要時在同一交易同步延長保存期限；不縮短既有承諾；不得回報已延長查詢卻在原保存期限刪除資料。
...
發布狀態、鎖定狀態與學籍／年度封存狀態應分別定義其作用對象，不得用單一值隱含三者。Phase 7 須提交合法狀態轉換表，包含操作權限、前置條件、失敗行為與公開可見版本。公開端不得讀取草稿或尚未提交完成的計算結果。2026-09-25 核准 D-08：檢測或段考任一先發布為 `PROVISIONAL`，只計已發布分類；兩者皆明確發布才為 `FINAL`。全部 NOT_HELD 仍須明確發布，缺分不阻擋發布且不視為 0 分。
...
2026-09-24 核准 D-04 第一項：完全無有效分數時，平均與總分皆為 NULL（顯示「—」），不給名次；實際 0 分仍為有效分數且可參與排名。
...
計算使用原始有效分數及其筆數，不得先將科目群平均四捨五入後再平均；也不得平均三次定評平均值來代替學期全部有效分數平均。有效分數為 0 分時仍須計入有效筆數；無有效分數時平均沒有數值，不得以 0 代替。精確小數運算及驗收示例見 §73.2。

2026-09-24 使用者核准 D-04 統計母體：班級／全年段分開列示評量開始日在籍人數、LOCAL 參與人數、快照具排名資格人數，以及各指標有效分數人數。一般成績統計包含排除排名但有分數者，排名統計只含合格且有分數者；原校成績全部另列，不混入本校群體統計。
...
2026-09-24 使用者核准 D-04 比序缺值規則：進入國文／英文／數學總分比序時，有數值者優先於缺值（實際 0 分也優先）；雙方皆缺值則續比下一科。缺值保留 NULL，不轉成 0。科目總分使用當次計算範圍內該科的原始有效分數。
...
依 D-08，任一分類先發布時產生 `PROVISIONAL` 暫時排名；檢測、段考皆明確發布後產生 `FINAL` 正式排名。
...
2026-09-23 使用者核准 D-03：單次評量優先於學期，學期優先於學生預設；空值代表繼承，學生預設納入排名。明確的單次／學期納入可覆寫較上層排除。名單確認時固定最終資格及來源值快照；評量開始後轉入自下一次參加，撤銷轉班或更改目前設定不改既有快照。原校成績仍一律不參與本校排名，不受覆寫規則放寬。
...
Phase 1 已建立初版 schema、migration 與約束；實體對照、ER 圖及復原設計見 [資料模型紀錄](docs/DATABASE.md)。後續 Phase 的待決策以新 migration 擴充，至少涵蓋：
...
2026-09-23 使用者核准啟動本 Phase，並確認推理強度為 HIGH 或以上、接受 D-03 建議及 D-11 日期規則。Google OAuth 仍依 Phase 0 決策暫緩；本 Phase 只建立其必要資料結構，不實作登入或呼叫真實 AI。交付證據見 [Phase 1 紀錄](docs/PHASE_1.md)，模型與復原設計見 [DATABASE.md](docs/DATABASE.md)。
...
- 技術時間戳以 UTC Unix milliseconds 整數保存；業務日期以 `YYYY-MM-DD` 的 Asia/Taipei 日期保存，區間採 `[from, to)`。月／年期限依曆月／曆年，同日不存在時取目標月末日；其他門檻仍待 D-11 後續決策。
- 分數 `score_value` 以百分之一分整數保存，0～10000 對應 0.00～100.00，避免浮點誤差。`NORMAL` 必有數值；特殊狀態與 `UNENTERED`（尚未輸入）均無數值且不得納入平均。原校成績不得納入本校排名。
- 除 §41 核心表外，必要的學期排名覆寫、身分查詢雜湊版本、公開結果版本與 AI 引用關聯可用獨立表保存，以維持 FK、唯一性與版本一致性。不得提前實作 D-04～D-10 未核准的計算、發布、Purge 或權限政策。
- 身分證採 AES-256-GCM 與獨立 HMAC-SHA-256 金鑰；密文綁定 Student ID 作 authenticated data，密文及 lookup hash 各自保存金鑰版本。多版本 lookup hash 使用 `(key_version, hash)` 唯一鍵。輪替需保留舊金鑰與查詢雜湊，完成驗證與回填後才能退役；金鑰遺失不得以明文備援或重新生成不同金鑰假稱可解密。
- Migration 由 Drizzle 生成，FTS／跨列約束使用有 journal 的 custom migration；seed 與 migration 分開。本 Phase 僅在隔離本機 D1 模擬資料庫套用，保留重跑、失敗復原與 preflight 測試，不呼叫雲端 migration 或 deployment。
...
2026-09-23 使用者核准執行 Phase 2；沿用已確認的 HIGH 以上推理設定。此階段實作可注入授權介面的伺服器領域服務，僅在隔離 D1 測試呼叫，不新增對外管理 API 或真實登入流程。交付與驗證見 [Phase 2 紀錄](docs/PHASE_2.md)。
...
- 建立學年度時由操作者指定開始日、第二學期開始日與結束日；一次原子建立兩學期，避免自行假設學校行事曆。新年度成為目前年度，其他年度預設唯讀，仍可查詢與作為升班來源。
- 所有寫入採持久 Preview → 明確 Confirm，保存來源 revision、操作者、範圍與操作結果。確認時重驗授權、Session 與來源 revision；同一 Preview 重送回傳既有結果，失敗不得留下部分學籍或 Audit。內部批次建檔接受已解析的列，Excel／CSV 上傳、範本及完整 Import Job 流程留待 Phase 6。
- 升班預設 7→8、8→9，沿用班級後兩碼及座號；目標班級須先建立，預覽可改派同目標年級班級與座號。九年級不自動升為不存在的十年級，也不藉升班執行畢業封存。
- 轉班與座號調整不得落在評量 `[starts_on, ends_on)` 期間。以撤銷舊有效學籍、建立必要前段與新段的方式保存修正歷程；已存在的評量仍指向原學籍與快照，不改寫成績、排名、History 或 AI 內容。
...
2026-09-24 使用者核准開始 Phase 4。沿用 D-03 的名單／資格快照與 D-10 的 Permission／Scope；實作目前學年度的草稿評量、科目設定、名單 Preview／Confirm 與草稿成績。已發布、鎖定、封存或歷史年度的寫入不得走草稿入口；解鎖、重算與發布流程於 Phase 7 銜接。Google／Sites 真實平台驗證仍依 D-09 暫緩。

本機實作與驗證紀錄見 [PHASE_4.md](docs/PHASE_4.md)。

本階段的評量與科目設定作用於整個學期評量，須有該學期全校範圍的 `score.write`；名單確認依班級範圍，單科成績依班級及科目範圍。名單由伺服器依評量開始日解析，不接受前端提供班級／資格快照。原校參與保留空本校班級快照，授權依伺服器解析的學生目前有效學籍，仍一律不參與本校排名。

草稿寫入使用評量及成績版本、操作識別碼與原子交易，保存成績修改歷程及 Audit；相同操作重送回傳原結果，不重複建立資料。已有本校成績的科目不能單獨切換 `held`，沿用 Phase 1 約束；單筆 `N` 不得改動全評量設定。此階段不實作平均、排名、匯入、發布或管理 UI。
...
2026-09-24 使用者核准啟動 Phase 5；依本節先建立測試再實作。D-04 的無有效分數、比序缺值與統計母體均已核准，規則見 §21／§22。

Phase 5 本機計算與授權讀取的驗證、交付邊界見 [PHASE_5.md](docs/PHASE_5.md)；不代表已發布成績。
...
2026-09-24 使用者核准啟動 Phase 6，並核准 D-07 與本階段 D-11 上傳門檻，依 §73.4 實作。本機驗證及交付邊界見 [PHASE_6.md](docs/PHASE_6.md)；不代表正式部署。
...
下列項目依各列狀態追蹤。D-01、D-03、D-10 及 D-11 日期部分已於 2026-09-23 由使用者核准；D-04、D-07、D-11 的 Recent Authentication／Recovery 時窗與 Phase 6 上傳門檻於 2026-09-24 核准，D-05、D-08 於 2026-09-25 核准，其餘建議仍不是已核准規則；實作到所列關卡前，必須先確認並回寫本節與相關正文。平台能力不足時保留實際證據，不可用模擬結果宣稱 Production 可用。
...
| D-05 | **已核准（2026-09-25）**：查詢延長必要時同步延長保存，僅可延長；須原因、Preview／Confirm 與 Audit | 有效轉出／畢業／延長事件的兩期限各取最晚；畢業不得縮短轉出承諾。誤標撤銷僅撤該事件且不覆蓋後續修改；正式恢復在籍暫停套用舊截止日，保留紀錄供後續事件比較 | Phase 8 |
...
| D-11 | **日期已核准（2026-09-23）；認證時窗與 Phase 6 上傳門檻已核准（2026-09-24）**；其餘門檻待決策 | 業務日期 Asia/Taipei；技術時間戳 UTC；含起不含迄；月／年期限依曆月／曆年，無對應日期時取該月最後一天。Google Recent Authentication 與一次性 Recovery 核准均 5 分鐘，滿時失效。CSV／XLSX：5 MiB、10 sheets、5,000 資料列、30 欄、展開 25 MiB／1,000 ZIP 項目；格式限制見 §73.4。查詢、重試、其他容量與復原目標仍待各功能 Phase 確認 | 日期於 Phase 1；認證時窗於 Phase 3B；匯入門檻於 Phase 6；其餘最晚 Phase 18 |
...
家長端僅可見該生自己的班級名次，不回傳其他學生名單或全年段個人排名。班級與全年段彙總統計依已核准 D-04 測試；公開查詢方式的殘餘風險須於 Phase 13／16 記錄。
...
| 全部為特殊狀態或未輸入 | 平均與總分皆為 NULL（顯示「—」），不排名；不能顯示為 0.00 |
...
- 平均、排名與公開結果必須對應同一成績版本；依 2026-09-25 核准 D-08，發布或修改重算失敗時整筆交易不生效，持續提供上一完整發布版本；首次發布失敗不公開結果。
...
- 2026-09-24 核准本階段 D-11：支援 UTF-8 CSV（可含 BOM）與 .xlsx；每檔最多 5 MiB、10 張工作表、合計 5,000 筆資料列、每列 30 欄；XLSX 展開上限 25 MiB／1,000 個 ZIP 項目。拒絕公式、巨集、外部連結、加密檔與舊版 .xls；超限整檔拒絕並顯示原因。
...
- Rollback 同樣須經預覽、授權、確認與稽核；依已核准 D-07，涉及發布後成績時阻擋匯入 Rollback。若另需修正已發布成績，走 §20 的解鎖、原因、History、重算及 AI stale 流程，不以 Rollback 略過。
- 2026-09-24 核准 D-07：Commit 與 30 天內 Rollback 均整批原子執行；任一列有錯即整批不寫入。匯入後若有手動修改、再次匯入或發布，先顯示衝突並阻擋整批回復；不得覆蓋後續合法修改。D-06 未確認前，不得讓 Purge 靜默破壞 30 天復原承諾。
...
以下定義各 Phase 的順序與最低交付證據；實際進度及已完成範圍以各 Phase 驗證紀錄與 README 為準。各 Phase 原章節的要求仍全部適用。
