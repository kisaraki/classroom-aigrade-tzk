...
- 轉出只處理尚無其他保存期限事件的 active 學生，保存轉出日期並依已定義規則設定 3 個曆年的查詢／保存期限；不封存、不刪除。已有其他保存事件或延長期限者，保留 D-05 關卡，不自行合併期限。
- 提供學籍新增、轉班／座號、升班與誤標轉出的撤銷預覽。撤銷必須核對受影響資料的提交後版本及內容，不覆蓋後續合法異動；保留已確認的評量快照。誤標轉出恢復其操作前狀態，與 D-05 尚待定案的跨事件重新入學分開。
...
下列項目依各列狀態追蹤。D-01、D-03、D-10 及 D-11 日期部分已於 2026-09-23 由使用者核准；D-04、D-07、D-11 的 Recent Authentication／Recovery 時窗與 Phase 6 上傳門檻於 2026-09-24 核准，D-08 於 2026-09-25 核准，其餘建議仍不是已核准規則；實作到所列關卡前，必須先確認並回寫本節與相關正文。平台能力不足時保留實際證據，不可用模擬結果宣稱 Production 可用。
